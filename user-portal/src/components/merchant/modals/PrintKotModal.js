...
import KotReceipt from '../common/KotReceipt';

export default function PrintKotModal({ isOpen, onClose, order }) {
        ...
        <div className="bg-card border border-border/40 rounded-2xl w-full max-w-lg p-6 relative flex flex-col space-y-4 shadow-2xl max-h-[95vh] overflow-y-auto">
          ...
          <div className="flex justify-between items-center pr-8 border-b border-border/40 pb-3">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground font-mono font-bold mt-0.5">Order ID: {order.orderId}</p>
            </div>
          ...
          <style>{`
            @media print {
              ...
              #kot-print-area {
                ...
                font-size: ${selectedPrintWidth === '58mm' ? '9px' : '11px'} !important;
